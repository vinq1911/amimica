...
func findFunctions(tokens []jsToken, src []byte) []funcSpan {
	...
	for i < len(tokens) {
		...
		braceEnd := matchDelimiter(tokens, bodyStart, "{", "}")
		if braceEnd < 0 {
			i++
			continue
		}
...
func matchFunction(tokens []jsToken, i int) (string, int, bool) {
	...
	if t.typ == jtIdent && i+off+1 < len(tokens) && tokens[i+off+1].val == "(" {
		pClose := matchDelimiter(tokens, i+off+1, "(", ")")
...
// matchDelimiter finds the index of the closing delimiter matching the opening
// one at start. Returns -1 if not found.
func matchDelimiter(tokens []jsToken, start int, open, close string) int {
